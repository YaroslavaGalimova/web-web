...
public class FrontServlet extends HttpServlet {
    ...
    private Method findMethod(Method method, String action, Class<?> pageClass) {
        if (method == null) {
            try {
                // System.err.println(pageClass.getDeclaredMethod(action, HttpServletRequest.class, Map.class));
                return pageClass.getDeclaredMethod(action, HttpServletRequest.class, Map.class);
    ...
    private void invokeMethods(List<Method> methods, Object page, HttpServletRequest request, Object view) throws IllegalAccessException, InvocationTargetException {
    ...
    private void process(Route route,
        ...
        Method before = null;
        Method after = null;
        for (Class<?> clazz = pageClass; (method == null || before == null || after == null) && clazz != null; clazz = clazz.getSuperclass()) {
            before = findMethod(before, "before", clazz);
            method = findMethod(method, route.getAction(), clazz);
            after = findMethod(after, "after", clazz);
        }
        ...
        try {
            invokeMethods(List.of(before, method, after), page, request, view);
